...
// 弁護士本人のカレンダーの予定を取得する（候補日時選びの画面で、何の予定が入っているか名称も含めて表示するため）
export async function getBusyEvents(timeMinIso, timeMaxIso) {
  const auth = await getAuth();
  const calendar = google.calendar({ version: 'v3', auth });
  ...
  return (res.data.items || [])
    ...
    .map((e) => ({ start: e.start.dateTime, end: e.end.dateTime, summary: e.summary || '（無題の予定）' }));
}
