...
async function readDB() {
  if (DB_URL) {
    ...
    if (rows.length === 0) {
      return { clients: [], rateTemplates: seedRateTemplates(), sessions: [] };
    }
    return rows[0].value;
  }
...
// 弁護士側が候補日時（2〜3個）を指定して、日程調整からの単一リンクを発行する。
// 相手が候補から1つ選ぶまでは pendingRequest に保持し、選ばれた時点でセッション（予約）に変換する。
export async function createScheduleRequest({ email, rateTemplateId, durationMinutes, candidates }) {
  const client = await findOrCreateClientByEmail(email);
  return updateClient(client.id, {
    pendingRequest: { rateTemplateId, durationMinutes, candidates },
  });
}
...
export async function createBooking({ clientId, rateTemplateId, scheduledAt, meetingCode, meetingUri, calendarEventId }) {
  const db = await readDB();
  const session = {
