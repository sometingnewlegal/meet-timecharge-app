...
- **Vercelへのpushが自動デプロイされないことがある。** 2026-07-10、GitHubにはpushできているのにVercelのDeploymentsに新しい行が出てこない事象が発生。原因はVercel側のGit連携（GitHub App経由、classic webhookではない）が何らかの理由で古いままになっていたこと。**直し方: Vercelの Settings → Git → 一度Disconnectしてから、Connect Git Repositoryで繋ぎ直す。** それでも直前のpush分は取りこぼされたままなので、`git commit --allow-empty`で空コミットを作ってpushし、再度Webhookを発火させて初めて新しいデプロイが作られた。今後同様に「pushしたのにVercelに反映されない」場合は、まずこの再接続→空コミットpushを試すこと。
...
## グラスモーフィズムへのデザイン刷新（2026-07-10、確定・本番反映済み）

ユーザーから明示的に指示があり、一気に全部やらず1段階ずつ、都度`npm run dev`のデモで確認しながら進めた。最終的に「この状態でfixしたい」と確定の指示があり、コミット・pushして本番（Vercel）にも反映済み（コミット`1f5251c` "Redesign the top page as a glassmorphism UI"、後続で空コミット`b8ceee1`）。
...
## カテゴリーの親コンテナ化（2026-07-10、確定・本番反映済み）
